...
final class WritingMainPostViewModel: BaseViewModel {
    ...
    private var locationUseCase = LocationUseCase.shared

    override init() {
        writingPostData = WritingPostData(location: locationUseCase.currentPlace)
        super.init()
...
extension WritingMainPostViewModel {
    func requestDataToUseCase() {
        inputs.locationChanged
            .map { coord in
                self.locationUseCase.geoCodeLocation(at: coord)
            }
            .compactMap { $0 }
            .flatMap { $0 }
...
extension WritingMainPostViewModel {
    func uiBusinessLogic() {
        ...
        outputs.boundaryLimit.onNext(locationUseCase.allowableBoundary)
