...
final class MyPageViewModel: BaseViewModel {
    ...
    private let dateUtil = DateUtil.shared
...
extension MyPageViewModel {
    func requestDataToUseCase() {
        routeInputs.needUpdate
            .filter { $0 }
            .flatMap { _ in self.postUseCase.myPage() }
        ...
        inputs.bookMark
            ...
            .flatMap { self.postUseCase.bookmark(postId: $0.ID, mark: !$0.marked) }
        ...
        inputs.attend
            ...
            .flatMap { self.postUseCase.attendance(postId: $0.ID) }
            .subscribe(onNext: { [weak self] result in
                switch result {
                case let .response(result: data):
                    guard data.success,
        ...
        inputs.photoSelected
            ...
            .flatMap { self.userUseCase.setProfileImage(to: $0) }
            .subscribe(onNext: { [weak self] result in
                switch result {
        ...
        inputs.changeToDefaultProfile
            .flatMap { self.userUseCase.setProfileImage(to: nil) }
            .subscribe(onNext: { [weak self] result in
                switch result {
                case .succeed:
